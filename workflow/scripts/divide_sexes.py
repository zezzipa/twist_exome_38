...
with open("qc/samtools_idxstats/idxstats_ratio.txt", "a") as f:
#	f.write("Sample\tRatio\tSex\n")
	for line in open("samples.tsv","r"):
		sample = line.split("\t")[0]
		if sample != "sample":
			for line in open("qc/samtools_idxstats/"+sample+"_N.samtools-idxstats.txt"):
				if "chrX" in line:
			...
			if sex < 0.03:
				...
				with open("qc/samtools_idxstats/unresolved_sex.txt", "a") as u:
